...
```{r}
# **ACTION ITEM**: Update currentyear as needed
currentyear <- 2025
...
# Load Crosswalks
```{r}
Crosswalk_FAFSA <- read.xlsx(file.path(base_dir,"Horizons","1.Data","Cleaned_Data","Regional_Analysis_Crosswalk.xlsx"),sheet="School District Names") 
...
Crosswalk_PS <- read.xlsx(file.path(base_dir,"Horizons","1.Data","Cleaned_Data","Regional_Analysis_Crosswalk.xlsx"),sheet="Horizon") %>% select(District.Code, SchoolCode, ERDC.SchoolTTL, ERDC.DistrictTTL, Horizons.Partner) %>% drop_na(Horizons.Partner) %>% distinct() 
Crosswalk_PS <- fix_district_code(Crosswalk_PS, column_name = "District.Code") 

Crosswalk_PS_Demo <- read.xlsx(file.path(base_dir,"Horizons","1.Data","Cleaned_Data","Regional_Analysis_Crosswalk.xlsx"),sheet="PS_Region") %>%
...
FAFSA_21_current <- FAFSA_21_current %>% #Clean names
  mutate(Program.Partnership = recode(Program.Partnership,
                                   'West Sound STEM (PREP)' = 'PREP', 'United Way of the Blue Mountains' = 'Elevate','Puget Sound ESD Partnership' = 'PSESD', 'United Way of Blue Mountains' = 'Elevate', 'ESD 112 Partnership' = 'ESD 112'
)) %>% mutate(Region.Name = recode(Region.Name,
                                   'West Sound STEM (PREP) Districts' = 'ALL PREP', 'United Way of Blue Mountains Districts' = 'ALL Elevate','Puget Sound ESD Partnership Districts' = 'ALL PSESD','ESD 112 Partnership Districts' = 'ALL ESD 112'
)) %>% 
...
Enrollment_HRP_12_long <- Enrollment_HRP_12_long %>%
  ...
  mutate(Horizons.Partner = case_when(
    Horizons.Partner == "West Sound STEM - Tier 2" & 
      SchoolYear %in% c("2014-2015", "2015-2016", "2016-2017", "2017-2018", "2019-2020") ~ "PREP - Tier 2",
    Horizons.Partner == "West Sound STEM - Tier 2" ~ "PREP", ## Note to self, Will need to add back Tier 2 once avail from WSAC.
    Horizons.Partner == "West Sound STEM" ~ "PREP",
...
Enrollment_HRP_12_ALL <- Enrollment_HRP_12Final %>% select(SchoolYear,DistrictCode,All.Students)

Horizon_Grads_Final <- Horizon_Grads_Final %>%
  left_join(Enrollment_HRP_12_ALL, by = c("SchoolYear", "DistrictCode")) %>%
  mutate(
    FinalCohort = if_else(is.na(FinalCohort), All.Students, FinalCohort),
...
### **ACTION ITEM**: Check for changed column names and update as needed, then run remainder of script
```{r}
## currently deactivated bc nothing saved to prior year folder due to this being new data processing
