...
### *Future consideration: add function to review changed distinct row values (district names, demographic group names, etc)*
```{r}
...
filter_horizons_partners <- function(df) {
  ...
  df %>%
    filter(Horizons.Partner %in% c(
      "ESD 112", "Puget Sound ESD", "United Way of the Blue Mountains", "West Sound STEM - Tier 1", "West Sound STEM - Tier 2", "PREP", "West Sound STEM", "PREP - Tier 1", "PREP - Tier 2"
    ))
}
...
#Function to split PREP into 2 tiers
replace_west_sound_stem <- function(df) {
  # Define the target District-SchoolCode combinations
  tier2_pairs <- data.frame(
    District.Code = c(18303, 18303, 18401, 18401, 18400, 18402, 18303, 18402,
                      18400, 18401, 18400, 18401, 18400, 18402, 18400),
    SchoolCode = c(2395, 1939, 5472, 2615, 5546, 3899, 1935, 1718,
                   5085, 4509, 3236, 4100, 1733, 2272, 1677)
  )

  # Convert to character if needed to match df column types
  tier2_pairs$District.Code <- as.character(tier2_pairs$District.Code)
  tier2_pairs$SchoolCode <- as.character(tier2_pairs$SchoolCode)

  # Also convert df columns if not already character
  df$District.Code <- as.character(df$District.Code)
  df$SchoolCode <- as.character(df$SchoolCode)

  # Create a join key for matching
  df$join_key <- paste0(df$District.Code, "_", df$SchoolCode)
  tier2_keys <- paste0(tier2_pairs$District.Code, "_", tier2_pairs$SchoolCode)

  # Apply the replacement
  df$Horizons.Partner <- ifelse(
    df$Horizons.Partner == "West Sound STEM" & df$join_key %in% tier2_keys,
    "PREP - Tier 2", ifelse(
      df$Horizons.Partner == "West Sound STEM",
      "PREP - Tier 1",
      df$Horizons.Partner
    )
  )
...
Crosswalk_PS <- replace_west_sound_stem(Crosswalk_PS)
...
Crosswalk_K12 <- replace_west_sound_stem(Crosswalk_K12)
```
...
```{r}
# Read in prior FAFSA data
## *Note: does not include Insight SD*
FAFSA_13_21 <- read.csv(file.path(base_dir,"Horizons", "1.Data","Cleaned_Data","FAFSA_10_Yr.csv")) %>%
  #match report date to current wsac file as excel 5 digit number, days since 1900
 mutate(
    ReportDate = as.Date(ReportDate, format = "%m/%d/%Y"),
    ReportDate = as.numeric(ReportDate - as.Date("1899-12-30"))
  )
FAFSA_13_21$FAFSA.Completion.Rate <- as.numeric(FAFSA_13_21$FAFSA.Completion.Rate)

# Read in new FAFSA data & K12 enrollment data. 
## *Note: FAFSA completion rate is provided directly to WASTEM from ERDC. Completion count is calculated using student count from enrollment file. Export contains all prior data with new data added each iteration shared from ERDC (FAFSA_21_current file excludes Insight and Open Doors SD) & doesn't include district code field, that gets added during crosswalk.*
FAFSA_21_current <- read.xlsx(file.path(base_dir,"Horizons/1.Data/Raw_Data", paste0(currentyear,"_Exports"),"FAFSA Completion by Subgroup Report for Horizons Partnerships.xlsx"),sheet = "FAFSA Completion by Subgroup Re") %>%
  ...
  mutate(High.School.Academic.Year = sub("^(//d{4})-//d{2}(//d{2})$", "//1-//2", High.School.Academic.Year)) %>% mutate(FAFSA.Completion.Rate = if_else(FAFSA.Completion.Rate == "Suppressed", '', FAFSA.Completion.Rate))
FAFSA_21_current$FAFSA.Completion.Rate <- as.numeric(FAFSA_21_current$FAFSA.Completion.Rate) 

FAFSA_21_current <- FAFSA_21_current %>% #Clean names
  mutate(Program.Partnership = recode(Program.Partnership,
                                   'West Sound STEM (PREP) Tier 2' = 'PREP - Tier 2','West Sound STEM (PREP) Tier 1' = 'PREP - Tier 1', 'United Way of the Blue Mountains' = 'Elevate','Puget Sound ESD Partnership' = 'PSESD', 'United Way of Blue Mountains' = 'Elevate', 'ESD 112 Partnership' = 'ESD 112'
)) %>% mutate(Region.Name = recode(Region.Name,
                                   'West Sound STEM (PREP) Tier 1 Districts' = 'ALL PREP - Tier 1', 'West Sound STEM (PREP) Tier 2 Districts' = 'ALL PREP - Tier 2', 'United Way of Blue Mountains Districts' = 'ALL Elevate','Puget Sound ESD Partnership Districts' = 'ALL PSESD','ESD 112 Partnership Districts' = 'ALL ESD 112'
)) %>% 
  # Rename method for All Students rows
  mutate(Demographic.Method = if_else(Demographic.Name == "All Students", 
                                       "All Students", 
                                       Demographic.Method))  %>%
  # Remove duplicates for each demographic group within each School Year + District
  filter(!(duplicated(paste(Demographic.Name, High.School.Academic.Year, Region.Name, FAFSA.Completion.Rate, Region.Type, ReportDate)))) 

# Keep only max report date per year 
FAFSA_max_filter <- FAFSA_21_current %>%
  distinct(ReportDate,High.School.Academic.Year) %>%
  group_by(High.School.Academic.Year) %>%
  summarize(ReportDate=max(ReportDate)) 

FAFSA_21_current_filtered <- FAFSA_21_current %>%
  inner_join(FAFSA_max_filter, by=c("High.School.Academic.Year"="High.School.Academic.Year","ReportDate"="ReportDate"))

# Join new and prior FAFSA data
FAFSA_all <- bind_rows(FAFSA_13_21,FAFSA_21_current_filtered) 

# Final clean of column names
FAFSA_Final <- FAFSA_all %>% rename(
"Horizons" = Program.Partnership, "Year" = High.School.Academic.Year) %>% select(-ReportDate)

FAFSA_Final$FAFSA_Year <- as.character(as.integer(substr(FAFSA_Final$Year, 1, 4)) + 1)    
FAFSA_Final <- FAFSA_Final %>% select(-Year)
     
# Modify Enrollment_HRP_12Final to match to FAFSA data
Enrollment_HRP_12_long <- Enrollment_HRP_12Final %>%
  select(SchoolYear,DistrictCode,Horizons.Partner,Gradelevel,Male, Asian, Native.Hawaiian.Other.Pacific.Islander, All.Students, Gender.X, Black.African.American, Two.or.More.Races, Female, American.Indian.Alaskan.Native, Hispanic.Latino.of.any.race.s., White) %>%
...
Enrollment_HRP_12_long <- Enrollment_HRP_12_long %>%
  ...
  ) %>% select(-Gradelevel) %>% #Clean HRP names
  mutate(Horizons.Partner = case_when(
    Horizons.Partner == "United Way of the Blue Mountains" ~ "Elevate",
    Horizons.Partner == "Puget Sound ESD" ~ "PSESD",
    TRUE ~ Horizons.Partner
  )) 

# Create regional totals
Enrollment_HRP_12_long_HRPTotal <- Enrollment_HRP_12_long %>% 
  group_by(SchoolYear,Horizons.Partner,Demographic.Name) %>% summarize(Students = sum(Students)) %>% mutate(DistrictCode = 'ALL')

# Join regional and district totals
Enrollment_HRP_12_long <- bind_rows(Enrollment_HRP_12_long,Enrollment_HRP_12_long_HRPTotal)

# Crosswalk to HRP
FAFSA_All_dist_codes <- join_to_hrp(
  df = FAFSA_all,
  ...
  mutate(District.Code = if_else(Region.Type == "Region", 
                                "ALL", 
                                District.Code))

# Add students to district 
## Leave nulls for suppressed completion rates
FAFSA_demos <- FAFSA_All_dist_codes %>% mutate("District Code" = as.character(District.Code)) %>% left_join(Enrollment_HRP_12_long,by=c("High.School.Academic.Year"="SchoolYear","District Code"="DistrictCode","Demographic.Name"="Demographic.Name", "Program.Partnership" = "Horizons.Partner")) %>%
  mutate(FAFSA.Completion.Rate=as.numeric(FAFSA.Completion.Rate),Completed=Students*FAFSA.Completion.Rate)  
# Rename columns to match tableau (WSAC SAP download)
## Leave nulls for 
WSAC_FAFSA <- FAFSA_demos %>%
  rename(
    Year = High.School.Academic.Year,
    `Demographic Value` = Demographic.Name,
    `Demographic Group` = Demographic.Method, 
    Cohort = Students, 
    `Estimated Completion` = Completed,
    Horizons = Program.Partnership,
    `Region Type` = Region.Type,
    `Region Name` = Region.Name
  ) %>%
  mutate(
    Year = sub("^(\\d{4})-(\\d{2})$", "\\1-20\\2", Year),
    Year = as.character(str_sub(Year, -4))
  )

# Calculate rate for all HRP
HRP_weighted_avgs <- WSAC_FAFSA %>% filter(`Region Type` == "Region") %>% filter(`Demographic Value` == "All Students") %>%   filter(!is.na(`Estimated Completion`), !is.na(Cohort), Cohort > 0) %>%  # exclude missing or zero Students
  group_by(Year) %>%
  summarise(
    `Estimated Completion` = sum(`Estimated Completion`, na.rm = TRUE),
    Cohort = sum(Cohort, na.rm = TRUE),
    FAFSA.Completion.Rate = `Estimated Completion` / Cohort,
    .groups = "drop"
  ) %>%
  mutate(
    Region.Type = paste("All HRP"), Horizons = paste("All HRP"), Region.Name = paste("All HRP"), Demographic.Name = paste("All Students")) %>% rename(FAFSA_Year = Year)

HRP_weighted_avgs_demos <- WSAC_FAFSA %>% filter(`Region Type` == "Region") %>%   filter(!is.na(`Estimated Completion`), !is.na(Cohort), Cohort > 0) %>%  # exclude missing or zero Students
  group_by(Year,`Demographic Group`, `Demographic Value`) %>%
  summarise(
    `Estimated Completion` = sum(`Estimated Completion`, na.rm = TRUE),
    Cohort = sum(Cohort, na.rm = TRUE),
    FAFSA.Completion.Rate = `Estimated Completion` / Cohort,
  ...
  mutate(
    `Region Type` = paste("All HRP"), Horizons = paste("All"), `Region Name` = paste("All HRP"))

# Bind final DFs back to HRP totals
FAFSA_Final <- bind_rows(FAFSA_Final, HRP_weighted_avgs)
WSAC_FAFSA <- bind_rows(WSAC_FAFSA, HRP_weighted_avgs_demos)
...
RC_Grad_WO_QVSD_5yr <- RC_Grad_WO_QVSD %>% filter(Cohort == "Five Year")  %>% 
  select(SchoolYear, DistrictCode, DistrictName, 
         StudentGroup,StudentGroupType, BeginningGrade9, FinalCohort, Graduate, GraduationRate) 
...
# Get 4-year and 5-year district totals for QVSD
...
RC_Grad_New_For_PS <- bind_rows(RC_Grad_WO_QVSD_5yr,RC_Grad_5yr)
...
#Replace null final cohort and graduates with estimated from enrollment data
##rename demo values in Enrollment file to match Grad file
rename_demos <- c(
  ...
  "Non.English.Language.Learners" = "Non-English Language Learners",
  "Foster.Care" = "Foster Care",
  "Non.Foster.Care" = "Non-Foster Care",
  "Highly.Capable" = "Highly Capable",
  "Non.Highly.Capable" = "Non-Highly Capable",
  "Non.Homeless" = "Non-Homeless",
  "Low.Income" = "Low-Income",
  "Non.Low.Income" = "Non-Low Income",
  "Non.Migrant" = "Non Migrant",
  "Military.Parent" = "Military Parent",
  "Non.Military.Parent" = "Non Military Parent",
  "Section.504" = "Section 504",
  "Non.Section.504" = "Non Section 504",
  "Students.with.Disabilities" = "Students with Disabilities",
  "Students.without.Disabilities" = "Students without Disabilities"
)

##pivot to long format to match grad file
Enrollment_HRP_12_long_demos_forPS <- Enrollment_HRP_12Final %>%
  select(-ERDC.DistrictTTL, -Mobile, -Non.Mobile) %>%
  pivot_longer(
    cols = -c(SchoolYear, Gradelevel, DistrictCode, Horizons.Partner),
    names_to = "Demographic.Name",
    values_to = "Students"
  ) %>%
  mutate(Demographic.Name = recode(Demographic.Name, !!!rename_demos))

Horizon_Grads_Final <- Horizon_Grads_Final %>%
  left_join(Enrollment_HRP_12_long_demos_forPS, by = c("SchoolYear", "DistrictCode", "Horizons.Partner", "StudentGroup" = "Demographic.Name")) %>%
  mutate(
    FinalCohort = if_else(is.na(FinalCohort), Students, FinalCohort),
    Graduate = if_else(is.na(Graduate), FinalCohort * GraduationRate, Graduate)
  ) %>% select(SchoolYear, DistrictCode, DistrictName, StudentGroup, StudentGroupType, FinalCohort, Graduate, GraduationRate, SchoolCode,SchoolName, CohortYearTTL, Horizons.Partner)
...
#rename demo values in First_Year_Enroll_All to match RC_Grad_Final
rename_demos <- c(
  "All" = "All Students",
  "Not Section 504" = "Non Section 504",
  #"Gender.X" = "Gender X",
  "American Indian or Alaska Native" = "American Indian/ Alaskan Native",
  "Black or African American" = "Black/ African American",
  "Hispanic or Latino of any race(s)" ="Hispanic/ Latino of any race(s)",
  "Native Hawaiian and Other Pacific Islander" =  "Native Hawaiian/ Other Pacific Islander",
  "FRPL" =  "Low-Income", 
  "Not FRPL" = "Non-Low Income", 
  "Bilingual" = "English Language Learners", 
  "Not Bilingual" = "Non-English Language Learners"
  #Note to self, ERDC confirmed no direct link between ERDC/OSPI for LAP, Special Education, and Disability
)

First_Year_Enroll_All <- First_Year_Enroll_All %>%  mutate(StudentGroup = recode(DemographicValue, !!!rename_demos))
...
PS_Enroll <- Horizon_Grads_Final %>% inner_join(First_Year_Enroll_Final,
  by = c("CohortYearTTL", "DistrictCode", "StudentGroup")) %>%
  mutate(
...
Horizon_Grads_All_Students <- Horizon_Grads_Final %>% filter(StudentGroupType == "All")
Enroll_By_Inst <- inner_join(
  Enroll_By_Inst_Final %>% mutate(CohortYearTTL = as.character(CohortYearTTL)),
  Horizon_Grads_All_Students,
...
PS_Enroll_For_Join <- PS_Enroll %>% select(-Pct,-GraduationRate,-RedactedPct) %>% filter(DemographicValue == "All")
...
PS_Enroll_Final <- PS_Enroll %>% select(DistrictCode,DistrictName,FinalCohort,Graduate,GraduationRate,CohortYearTTL,Horizons.Partner,PSEnrollLevel,Pct,`Cohort Enrollment Rate`, DemographicGroup, DemographicValue)
...
### []Poppe updating
```{r}
# download from IPEDS, changed file name to IPEDS_long.csv
## note to self, does anything need to change on this to include students originating from Columbia (Walla Walla) SD for Elevate?

## adding IPEDS data to EACH Horizons partner that, users will see any institutions students attend from their network [IPDES data source](https://nces.ed.gov/ipeds/datacenter/InstitutionByName.aspx?sid=30ef727a-63d8-40ce-a1f1-e7dad2233c1e&rtid=1)
...
required_districts <- c(
  ...
  "18400", "18401", "18402", "18902", "20400", "20401", "20402", "20405", "36400",
...
str(SankeyDataModel)
SankeyDataModel_CHECK <- SankeyDataModel %>% select(CohortYear,PSOrg, District, Horizon) %>% distinct() %>% left_join(Crosswalk_K12, by =c('District' = 'ERDC.DistrictTTL'))

SankeyDataModel_Missing_Dis <- find_missing_districts(SankeyDataModel_CHECK,
  year_col = "CohortYear",
...
str(WSAC_FAFSA)
WSAC_FAFSA_CHECK <- WSAC_FAFSA %>% select(`Year`,`District Code`, Horizons) %>% distinct()
...
# FAFSA_Final.xlsx
str(FAFSA_Final)
FAFSA_Final_CHECK <- FAFSA_All_dist_codes %>% 
  select(High.School.Academic.Year, `District.Code`, Program.Partnership) %>% 
  distinct()

FAFSA_Final_Missing_Dis <- find_missing_districts(
  FAFSA_Final_CHECK,
  year_col = "High.School.Academic.Year",
  district_col = "District.Code"
)
...
Missing_Districts_All <- bind_rows(
  ...
  SankeyDataModel_Missing_Dis %>% mutate(Source = "SankeyDataModel_Missing_Dis")
)
